...
# -- Q1. Find out the top three product id based on 
...
# Q2. Get the order date, product id, product name and 
# the order id which has been sold in the multiples of 2
select * from order_lines;
select * from orders;
select * from products;
select o.order_date , p.product_id , p.product_name , ol.order_id from
orders as o join products as p join order_lines as ol 
on p.product_id = ol.product_id where ol.quantity%2=0;

# Q3. Get the brand name, order value, product name
...
join orders o join products p on b.product_id = p.product_id group by
b.brand, o.order_value,p.product_name;
